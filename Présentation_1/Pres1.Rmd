...
En 2010, Grün, Kosmidis et Zeilis ont publié l'article *Extended Beta Regression in R: Shaken, Stirred, Mixed and Partitioned*, dans lequel ils expliquent différentes méthodes plus complexes pouvant améliorer le modèle. Trois grois thèmes y ressortent, soient : 
...
Dans l'article, on discute le fait que la méthode du maximum de vraisemblance pour estimer les paramètres de $\phi$ ont tendance à sous-estimer les écarts-types des paramètres, ce qui amène des problèmes notamment au niveau de l'inférence que l'on peut faire avec le modèle. 
...
Remarquons le ```type``` correspond à la mtéhode d'estimation choisie.
...
On pourrait  qu'il y a des différences dans différents sous-groupes de l'échantillon, mais qu'il existe pas de variables discriminante. La solution : utiliser ```betamix```. Voici un example. Ça revient à créer un 3 _clusters_ de données et d'estimer un modèle pour chacun des _clusters_.
